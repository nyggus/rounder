...
if __name__ == "__main__":
    ...
    x = collections.namedtuple("x", "a b")(1, 2.66)

    y = typing.NamedTuple("y", [("a", float), ("b", float)])(1, 2.66)

    c = collections.OrderedDict({1: 2.67})
    yc()
    yc(round_object([x, y, c]))

    class X(collections.UserDict):
        pass

    class Y(collections.UserList):
        pass
    ...
    x = X({1: 2.45, 3: 4.55})
    print(x)
    print(x, round_object(x, use_copy=True))

    y = Y([1, 2, 3.56])
    print(y, round_object(y), type(round_object(y)))

    x = collections.deque()
    x.append(1)
    x.append(2.52727)
    print(round_object(x))

    z = Z((1, 2, 3.5))
    print(round_object(z), type(round_object(z)))

    d = collections.Counter(["egs", "ham"])
    for k, v in d.items():
        d[k] = v + 2.2
    print(d, type(d), isinstance(d, dict))
    print(round_object(d))

    print(round_object(["abc", 12.45, 14.2, {"a": 2.34, "b": 10.6}]))

    print(round_object(fractions.Fraction(4.75), 1))

    c = 2.34 + 4.45j
    print(round_object([c, 2.34]))
    print(round(2.34))
    ...
    #    _ = round_object(gen, use_copy=True)
    _ = round_object(gen, use_copy=False)

    print("______")

    yc(round_object(10.2))
